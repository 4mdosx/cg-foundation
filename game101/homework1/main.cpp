...
{
    // n, f: z of near/far plane in view space (camera looks along -Z, so negative)
    float n = -zNear, f = -zFar;
    float fov_rad = eye_fov * static_cast<float>(MY_PI) / 180.0f;
    float t = std::tan(fov_rad / 2.0f) * std::abs(n);
